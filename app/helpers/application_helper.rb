...
module ApplicationHelper
  ...
  #区分登陆账号，并修改密码
  def update_diff_password
    if session[:account_id]
      # return new_admin_account_path
    end
  end
  ...
  def quick_url_options
    current_site = RegionUser.find(session[:region_user_id]).region.site
    # @mo_url = "http://mkong.dajiaxiao.cn/mo"
    # @topic_url = "http://#{@wx_mp_user.site.id}.mkong.dajiaxiao.cn/wap/topics"
    # @user_url = "http://#{@wx_mp_user.site.id}.mkong.dajiaxiao.cn/wap/infos/try"
    {
      '请选择' => '',
      "MO入口" => "http://#{Settings.hostname}/mo",
      "互动游戏" => "http://#{current_site.id}.#{Settings.mhostname}/wap/games",
      "群聊入口" => "http://#{current_site.id}.#{Settings.mhostname}/wap/topics/display",
      "个人中心" => "http://#{current_site.id}.#{Settings.mhostname}/wap/myhomes/transfer",
      "积分商城入口" => "http://#{current_site.id}.#{Settings.mhostname}/wap/infos/try"
